                ...
                <div class="bg-gray-100 dark:bg-gray-700 rounded-lg">
                    <div class="flex items-center justify-between p-4">
                        ...
                        <div class="flex space-x-2">
                            @if($index > 0)
                                <button wire:click="moveCategoryUp({{ $category['id'] }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">
                                    Move Up
                                </button>
                            @endif
                            @if($index < count($categories) - 1)
                                <button wire:click="moveCategoryDown({{ $category['id'] }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">
                                    Move Down
                                </button>
                            @endif
                            <button wire:click="editCategory({{ $category['id'] }})" class="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-200">
                                Edit
                            </button>
                            <button wire:click="confirmDeleteCategory({{ $category['id'] }})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-200">
                                Delete
                            </button>
                        </div>
                    </div>
                            ...
                            <div class="flex items-center justify-between py-2 px-2 bg-gray-50 dark:bg-gray-700 rounded-md">
                                <div class="flex items-center">
                                    @if($field['type'] == 'header')
                                        <span class="font-semibold text-gray-800 dark:text-gray-100 text-lg">{{ $field['content'] }}</span>
                                    ...
                                    @else
                                        ...
                                        <span class="text-gray-500 dark:text-gray-400 ml-2">({{ $field['type'] }})</span>
                                    @endif
                                </div>
                                <div class="flex space-x-2">
                                    @if($fieldIndex > 0)
                                        <button wire:click="moveFieldUp({{ $field['id'] }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">
                                            Move Up
                                        </button>
                                    @endif
                                    @if($fieldIndex < count($category['fields']) - 1)
                                        <button wire:click="moveFieldDown({{ $field['id'] }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">
                                            Move Down
                                        </button>
                                    @endif
                                    <button wire:click="editField({{ $field['id'] }})" class="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-200">
                                        Edit
                                    </button>
                                    <button wire:click="confirmDeleteField({{ $field['id'] }})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-200">
                                        Delete
                                    </button>
                                </div>
                            </div>
                    ...
                    <form wire:submit.prevent="updateCategory">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="mb-4">
                                ...
                                @error('categoryBeingEdited.name') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                            <div class="mb-4">
                                ...
                                @error('categoryBeingEdited.description') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                            <div class="mb-4">
                                ...
                                @error('categoryBeingEdited.percentage_start') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                            <div class="mb-4">
                                ...
                                @error('categoryBeingEdited.percentage_end') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <form wire:submit.prevent="updateField">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="mb-4">
                                ...
                                @error('fieldBeingEdited.type') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                            ...
                            @if(in_array($fieldBeingEdited['type'], ['header', 'description']))
                                <div class="mb-4">
                                    ...
                                    @error('fieldBeingEdited.content') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                                </div>
                            @else
                                <div class="mb-4">
                                    ...
                                    @error('fieldBeingEdited.label') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                                </div>
                                ...
                                @if(in_array($fieldBeingEdited['type'], ['select', 'checkbox', 'radio']))
                                    <div class="mb-4">
                                        ...
                                        @error('fieldBeingEdited.options') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                                    </div>
                                @endif
